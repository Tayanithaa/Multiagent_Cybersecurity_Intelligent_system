"""
Train RoBERTa model for correlation prediction.
Binary classification: should_correlate (0/1).
"""
import json
...
def main():
    ...
    train_dataset = Dataset.from_dict(
        {"text": train_df["correlation_text"].tolist(), "label": train_df["label"].tolist()}
    )
    val_dataset = Dataset.from_dict(
        {"text": val_df["correlation_text"].tolist(), "label": val_df["label"].tolist()}
    )
    test_dataset = Dataset.from_dict(
        {"text": test_df["correlation_text"].tolist(), "label": test_df["label"].tolist()}
    )
    ...
    print(
        classification_report(
            true_labels, predictions, target_names=["no", "yes"], digits=4
        )
    )
    ...
    eval_results = {
        ...
        "classification_report": classification_report(
            true_labels, predictions, target_names=["no", "yes"], output_dict=True
        ),
